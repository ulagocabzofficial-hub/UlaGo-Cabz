...
async function handleBooking(event, bookingType) {
    ...
    try {
        ...
        const h = payload['Pickup_Hour'], m = payload['Pickup_Minute'], ap = payload['Pickup_AMPM'];
        if (h && m && ap) {
            payload['Pickup_Time'] = h + ':' + m + ' ' + ap;
            delete payload['Pickup_Hour']; delete payload['Pickup_Minute']; delete payload['Pickup_AMPM'];
        }
